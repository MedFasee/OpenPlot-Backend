...
public static class RunsEndpoints
{
    ...
    ";
        ...
        // -------------------------------
        // 1.5) /series/unified/by-run
        // -------------------------------
        grp.MapGet("/series/unified/by-run",
        async Task<IResult> (
            HttpContext http,
            [AsParameters] UnifiedSeriesQuery q
        ) =>
        {
            if (q.RunId == Guid.Empty)
                return Results.BadRequest("runId é obrigatório.");

            var route = ResolveSeriesRoute(q);
            if (route is null)
                return Results.BadRequest($"Combinação ainda não suportada: quantity={q.Quantity}, component={q.Component}, phaseMode={q.PhaseMode}");
            ...
            if (http.Request.Headers.TryGetValue("Cookie", out var cookies) && !string.IsNullOrWhiteSpace(cookies))
            {
                req.Headers.TryAddWithoutValidation("Cookie", cookies.ToString());
            }

            using var client = new HttpClient();
            using var resp = await client.SendAsync(req, HttpCompletionOption.ResponseHeadersRead, http.RequestAborted);
            ...
            {
                return Results.Content(body, resp.Content.Headers.ContentType?.MediaType ?? "application/json");
            }

            if (node is not JsonObject obj)
                return Results.Content(body, "application/json");

            obj["labels"] = new JsonObject
            {
                ["title"] = BuildChartTitle(q),
                ["xAxisTitle"] = BuildXAxisTitle(obj["data"]?.GetValue<string>()),
                ["yAxisTitle"] = BuildYAxisTitle(q)
            };
...
";
    ...
    private static string? ResolveSeriesRoute(UnifiedSeriesQuery q)
    {
        var quantity = (q.Quantity ?? "").Trim().ToLowerInvariant();
        var component = (q.Component ?? "").Trim().ToLowerInvariant();
        var phaseMode = (q.PhaseMode ?? "").Trim().ToLowerInvariant();

        if (component == "angle" && (quantity == "voltage" || quantity == "current"))
            return "/series/angle-diff/by-run";

        if (phaseMode == "deseq" && component == "mag" && (quantity == "voltage" || quantity == "current"))
            return "/series/unbalance/by-run";

        if (IsSeqMode(phaseMode) && component == "mag" && (quantity == "voltage" || quantity == "current"))
            return "/series/seq/by-run";

        if (quantity == "voltage" && component == "mag" && !IsSeqMode(phaseMode))
            return "/series/voltage/by-run";

        if (quantity == "current" && component == "mag" && !IsSeqMode(phaseMode))
            return "/series/current/by-run";

        if (quantity == "p_active" || quantity == "p_reactive")
            return "/series/power/by-run";

        if (quantity == "frequency")
            return "/series/frequency/by-run";

        if (quantity == "dfreq")
            return "/series/dfreq/by-run";

        if (component == "thd" && (quantity == "voltage" || quantity == "current"))
            return "/series/thd/by-run";

        if (quantity == "digital")
            return "/series/digital/by-run";

        return null;
    }

    private static List<KeyValuePair<string, string>> BuildTargetQuery(UnifiedSeriesQuery q)
    {
        ...
        if (component == "angle" && (quantity == "voltage" || quantity == "current"))
        {
            list.Add(new("run_id", q.RunId.ToString()));
            list.Add(new("Tri", "false"));
            list.Add(new("kind", quantity));
            if (!string.IsNullOrWhiteSpace(q.ReferenceTerminal)) list.Add(new("ref", q.ReferenceTerminal.Trim()));
            var seq = MapSeqParam(phaseMode);
            if (!string.IsNullOrWhiteSpace(seq)) list.Add(new("seq", seq));
            else if (!string.IsNullOrWhiteSpace(q.Phase)) list.Add(new("phase", q.Phase.Trim()));
            AddPeriod(q, list);
            AddPmus(list, q.Terminals, q.ReferenceTerminal);
            return list;
        }
        ...
        if (phaseMode == "deseq" && component == "mag" && (quantity == "voltage" || quantity == "current"))
        {
            list.Add(new("run_Id", q.RunId.ToString()));
            list.Add(new("kind", quantity));
            if (quantity == "voltage" && string.Equals(q.Unit, "pu", StringComparison.OrdinalIgnoreCase)) list.Add(new("unit", "pu"));
            AddPeriod(q, list);
            AddPmus(list, q.Terminals);
            return list;
        }
        ...
        if ((quantity == "voltage" || quantity == "current") && component == "mag" && !IsSeqMode(phaseMode))
        {
            var isTri = phaseMode == "three_phase";
            list.Add(new("run_Id", q.RunId.ToString()));
            list.Add(new("Tri", isTri ? "true" : "false"));
            if (!isTri && !string.IsNullOrWhiteSpace(q.Phase)) list.Add(new("phase", q.Phase.Trim()));
            if (isTri && q.Terminals?.Length > 0 && !string.IsNullOrWhiteSpace(q.Terminals[0])) list.Add(new("id_name", q.Terminals[0].Trim()));
            if (quantity == "voltage" && string.Equals(q.Unit, "pu", StringComparison.OrdinalIgnoreCase)) list.Add(new("unit", "pu"));
            AddPeriod(q, list);
            return list;
        }
        ...
        {
            list.Add(new("run_id", q.RunId.ToString()));
            list.Add(new("which", quantity == "p_active" ? "active" : "reactive"));
            list.Add(new("unit", "raw"));
            list.Add(new("total", "true"));
            AddPeriod(q, list);
            AddPmus(list, q.Terminals);
            return list;
        }

        if (quantity == "frequency" || quantity == "dfreq")
        {
            list.Add(new("run_Id", q.RunId.ToString()));
            AddPeriod(q, list);
            return list;
    ...
    private static void AddPmus(List<KeyValuePair<string, string>> query, string[]? terminals, string? except = null)
    {
        if (terminals is null) return;
        foreach (var terminal in terminals.Where(t => !string.IsNullOrWhiteSpace(t)))
        {
            var value = terminal.Trim();
            if (!string.IsNullOrWhiteSpace(except) && string.Equals(value, except.Trim(), StringComparison.Ordinal)) continue;
            query.Add(new("pmu", value));
        }
    }

    private static void AddPeriod(UnifiedSeriesQuery q, List<KeyValuePair<string, string>> query)
    {
        if (q.InicioIso.HasValue) query.Add(new("from", q.InicioIso.Value.ToUniversalTime().ToString("O")));
        if (q.FimIso.HasValue) query.Add(new("to", q.FimIso.Value.ToUniversalTime().ToString("O")));
    }

    private static bool IsSeqMode(string? phaseMode)
    {
        var value = (phaseMode ?? "").Trim().ToLowerInvariant();
        return value is "pos_seq" or "seq_pos" or "seq_neg" or "seq_zero" or "deseq";
    }

    private static string? MapSeqParam(string? phaseMode)
    {
        return (phaseMode ?? "").Trim().ToLowerInvariant() switch
        {
            "pos_seq" or "seq_pos" => "pos",
            "seq_neg" => "neg",
            "seq_zero" => "zero",
            _ => null
        };
    ...
    private static string BuildXAxisTitle(string? data)
        => $"Tempo (UTC){(!string.IsNullOrWhiteSpace(data) ? $" - Dia {data}" : "")}";

    private static string BuildYAxisTitle(UnifiedSeriesQuery q)
    {
        var quantity = (q.Quantity ?? "").Trim().ToLowerInvariant();
        var component = (q.Component ?? "").Trim().ToLowerInvariant();
        var phaseMode = (q.PhaseMode ?? "").Trim().ToLowerInvariant();
        ...
        if (quantity == "voltage" && component != "thd" && phaseMode != "deseq")
            return component == "angle" ? "Diferença Angular (Graus)" : string.Equals(q.Unit, "pu", StringComparison.OrdinalIgnoreCase) ? "Tensão (pu)" : "Tensão (V)";

        if (quantity == "current" && component != "thd" && phaseMode != "deseq")
            return component == "angle" ? "Diferença Angular (Graus)" : "Corrente (A)";

        return quantity switch
        {
            "p_active" => "Potência Ativa (MW)",
            "p_reactive" => "Potência Reativa (MVAr)",
            "frequency" => "Frequência (Hz)",
            "dfreq" => "Var. de Frequência (Hz/s)",
            "digital" => "Nível",
            _ when component == "thd" => "Distorção Harmônica (%)",
            _ when phaseMode == "deseq" => "Desequilíbrio (%)",
            _ => "Grandeza"
        };
    }

    private static string BuildChartTitle(UnifiedSeriesQuery q)
    {
        var quantity = (q.Quantity ?? "").Trim().ToLowerInvariant();
        var component = (q.Component ?? "").Trim().ToLowerInvariant();
        var phaseMode = (q.PhaseMode ?? "").Trim().ToLowerInvariant();
        var term0 = q.Terminals?.FirstOrDefault(t => !string.IsNullOrWhiteSpace(t))?.Trim();

        var suffix = FormatResolution(q.ResolutionSearch);
        suffix = string.IsNullOrWhiteSpace(suffix) ? "" : $" - {suffix}";

        if (quantity == "frequency") return "Frequência" + suffix;
        if (quantity == "dfreq") return "Variação de Frequência" + suffix;
        if (quantity == "p_active") return "Potência Ativa" + suffix;
        if (quantity == "p_reactive") return "Potência Reativa" + suffix;
        if (quantity == "digital") return "Sinal Digital" + suffix;
        if (component == "thd")
        {
            var qn = quantity == "voltage" ? "Tensão" : quantity == "current" ? "Corrente" : quantity;
            return "Distorção de " + qn + " Harmônica Total" + suffix;
        }
        if (phaseMode == "deseq") return "Desequilíbrio de " + (quantity == "voltage" ? "Tensão" : quantity == "current" ? "Corrente" : "Grandeza") + suffix;
        if (component == "angle")
        {
            var qn = quantity == "voltage" ? "Tensão" : quantity == "current" ? "Corrente" : quantity;
            return "Diferença Angular da " + qn + " - Ref.: " + q.ReferenceTerminal + suffix;
        }
        if (phaseMode == "three_phase" && !string.IsNullOrWhiteSpace(term0))
            return $"Módulo da {(quantity == "voltage" ? "Tensão" : quantity == "current" ? "Corrente" : "Grandeza")} - {term0}{suffix}";

        var domain = phaseMode switch
        {
            "single" when !string.IsNullOrWhiteSpace(q.Phase) => $"Fase {q.Phase}",
            "three_phase" => "Trifásico",
            "seq_pos" or "pos_seq" => "Sequência Positiva",
            "seq_neg" => "Sequência Negativa",
            "seq_zero" => "Sequência Zero",
            "deseq" => "Desequilíbrio",
            _ => ""
        };

        var baseTitle = (component == "mag" ? "Módulo" : component == "angle" ? "Ângulo" : "Grandeza") + " da " +
                        (quantity == "voltage" ? "Tensão" : quantity == "current" ? "Corrente" : "Grandeza");

        if (!string.IsNullOrWhiteSpace(domain)) baseTitle += " - " + domain;
        return baseTitle + suffix;
    }

    private static string FormatResolution(string? resolutionSearch)
    {
        if (string.IsNullOrWhiteSpace(resolutionSearch)) return "";
        var s = resolutionSearch.Trim();
        if (s.Contains("raw", StringComparison.OrdinalIgnoreCase)) return "";
        if (double.TryParse(s, NumberStyles.Any, CultureInfo.InvariantCulture, out var n) && n > 0)
        {
            if (Math.Abs(n - 1.0) < 0.0001) return "1 fasor/s";
            return $"{n:0.####} fasores/s";
        }
        return "";
    }
