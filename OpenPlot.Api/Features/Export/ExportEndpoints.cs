    ...
    public static IEndpointRouteBuilder MapExport(this IEndpointRouteBuilder app)
    {
        ...
        ) =>
        {
            ...
            if (!Guid.TryParse(runIdRaw, out var runId) || runId == Guid.Empty)
                return Results.BadRequest("run_id inv�lido");

            var format = req.format?.Trim();
            if (string.IsNullOrWhiteSpace(format))
                return Results.BadRequest("format � obrigat�rio");

            if (!IsSupportedFormat(format))
                return Results.BadRequest(new { error = "Formato de exporta��o ainda n�o suportado", format });
...
LIMIT 1;", new { run_id = runId });
            ...
            if (runStatus is null)
                return Results.NotFound("run n�o encontrada.");
            ...
            // Processa exportação sincronamente com timeout de 60 segundos
            try
            {
            ...
            catch (OperationCanceledException)
            {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        // Log de erro em background é capturado pelo processador
                    }
        ...
        ) =>
        {
            ...
            if (!string.Equals(row.status, "done", StringComparison.OrdinalIgnoreCase))
                return Results.BadRequest(new { error = "export ainda n�o conclu�do", status = row.status, progress = row.progress });
