...
public interface IPronyMetaBuilder
{
    PlotMetaDto Build(RowsCacheV2 payload, DateTime? fromUtc = null, DateTime? toUtc = null);
}

public sealed class PronyMetaBuilder : IPronyMetaBuilder
{
    private readonly IPlotMetaBuilder _plotMetaBuilder;

    public PronyMetaBuilder(IPlotMetaBuilder plotMetaBuilder)
    {
    ...
    public PlotMetaDto Build(RowsCacheV2 payload, DateTime? fromUtc = null, DateTime? toUtc = null)
    {
        if (payload.Series is null || payload.Series.Count == 0)
            return new PlotMetaDto("Prony", "Tempo (UTC)", "Valor");
        ...
        return new PlotMetaDto(
            Title: BuildPronyTitle(baseMeta.Title),
            XLabel: baseMeta.XLabel,
            YLabel: baseMeta.YLabel);
    }
    ...
    private static string BuildPronyTitle(string baseTitle)
    {
        var article = UsesFeminineArticle(baseTitle) ? "da" : "do";
        return $"Prony {article} {baseTitle}";
    }
